...
# マップ保存、ロードツール
# -----------------------------------------------------------------------------
def save_graph_map(G, nodes, edges, skeleton, output_dir=GRAPH_MAP_DIR):
...
def ret_routed_drivingline(drivingline_x1, MG, start_state, goal_state):
    ...
    np.save('tmp/routed_road.npy', routed_drivingline)
    Image.fromarray((np.rot90(routed_drivingline, k=1)).astype(np.uint8)).save('tmp/routed_road.png')
...
routed_drivingline = ret_routed_drivingline(drivingline_x1, MG, start_state, goal_state)
# %%
print_movements_at_node(MG, 57)
# %%
print_connected_edges(G, 123)
# %%
